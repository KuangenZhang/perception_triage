...
def sql_configuration(sql_query: str = "", new_col_sql: str = "") -> tuple[str, str]:
    if not sql_query:
    ...
    with col1:
        ...
        sql_query = st.text_area("Input SQL Query", value=sql_query, height=100)
        if st.button("Apply SQL"):
            apply_sql_query(sql_query)

    with col2:
        st.subheader("Add Computed Column")
        new_col_sql = st.text_input("SQL for new column", placeholder=new_col_sql)
    ...
    for idx, col_name in enumerate(st.session_state.current_df.columns):
        with cols[idx % 3]:
            ...
            st.session_state.labels[col_name] = st.text_input(
                f"Label for {col_name}", value=current_label, key=f"label_{col_name}"
            )


def pagination_controls(df: pd.DataFrame) -> int:
    assert len(df) > 0
    total_pages = int(math.ceil(len(df) / st.session_state.rows_per_page))
    with st.sidebar:
...
def calc_column_widths(df: pd.DataFrame, img_column_width_sum: float = 0.5) -> Any:
...
def display_data_preview() -> None:
    ...
    if st.session_state.sort_column:
        df = sort_dataframe(
            df, st.session_state.sort_column, st.session_state.sort_ascending
        )
...
def save_current_df(df: pd.DataFrame) -> None:
    ...
    if st.sidebar.button("Download table"):
        ...
        st.success(
            f"CSV is successfully downloaded to {csv_path}. Images are copied to {dst_img_folder}"
        )


def split_img_paths(processed_df: pd.DataFrame) -> pd.DataFrame:
...
def add_img_dst_paths(processed_df: pd.DataFrame, dst_img_folder: str) -> pd.DataFrame:
    for i, model_version in enumerate(st.session_state.model_versions):
        processed_df[f"img_uuid_{i}"] = (
            model_version + "_" + processed_df[f"frame_id"].astype(str)
        )
        processed_df[f"img_dst_{i}"] = (
            dst_img_folder + "/" + processed_df[f"img_uuid_{i}"] + ".png"
        )
    return processed_df
