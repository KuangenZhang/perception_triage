...
from data_utils import copy_src_imgs_to_dst, download_files, wandb_table_to_csv


def combine_tables(
    df_list: list[pd.DataFrame], cache_img_column_name: str, metrics_for_diff: str
) -> pd.DataFrame:
    ...
    df_combined[f"{cache_img_column_name}"] = (
        df_list[0][cache_img_column_name].astype(str)
        + ","
        + df_list[1][cache_img_column_name].astype(str)
    )
...
def get_args() -> Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The metrics to calculate diff",
    )
    parser.add_argument(
        "-e", "--entity", type=str, default="zhangkevin", help="Entity name"
    )
    parser.add_argument(
        "-p", "--project", type=str, default="LaneDetection2D", help="Project name"
    )
    parser.add_argument(
        "-i",
        "--img_column_idx",
        type=int,
        default=0,
        help="Image column idx in the table",
    )
    parser.add_argument(
        "-t",
        "--table_name",
        type=str,
        default="predictions_table.table.json",
        help="Table name",
    )
...
def main() -> None:
    ...
    for model_version, run_id in zip(args.model_versions, args.run_ids):
        ...
        artifact_dir = download_files(
            args.entity, args.project, run_id, path_prefix=table_name
        )
        ...
        if os.path.exists(csv_path):
            ...
            wandb_table_to_csv(
                artifact_dir, table_name, csv_name, img_column_idx=args.img_column_idx
            )
            df = pd.read_csv(csv_path)
            img_column_name = df.columns[args.img_column_idx]
            assert (
                "frame_id" in df.columns
            ), "Error: 'frame_id' column is missing in the DataFrame."
            df[cache_img_column_name] = df["frame_id"].apply(
                lambda x: os.path.join(model_artifact_dir, "media", f"{x}.png")
            )
    ...
    df_combined_path = os.path.join(
        artifact_root_dir, "_".join(args.model_versions) + ".csv"
    )
